...
提供 `default`、`medium`、`mini`、`xmini` 四种尺寸。
...
通过 `display` 设置按钮的显示模式：`block` 填满父容器，`inline` 行内排列。
...
通过 `open-type` 设置微信小程序按钮的开放能力，如 `share`、`getPhoneNumber`、`contact` 等。
