...
test.describe('Progress 文档', () => {
  ...
  test('基础用法：inner-bar 宽度为 50%', async ({ page, gotoDocsPage }) => {
    ...
    await expect(innerBar).toHaveCSS('width', '50%')
  })
  ...
  test('不同进度：4 条进度条宽度依次为 0/30/60/100', async ({ page, gotoDocsPage }) => {
    ...
    await expect(innerBars.nth(0)).toHaveCSS('width', '0%')
    await expect(innerBars.nth(1)).toHaveCSS('width', '30%')
    await expect(innerBars.nth(2)).toHaveCSS('width', '60%')
    await expect(innerBars.nth(3)).toHaveCSS('width', '100%')
  })
  ...
  test('动态进度：点击 +20 按钮增加进度', async ({ page, gotoDocsPage }) => {
    ...
    await expect(dynamicDemo.locator('.weui-progress__inner-bar')).toHaveCSS('width', '30%')
    // 点击 +20 按钮（primary 按钮）
    await dynamicDemo.locator('.weui-btn_primary').click()
    await expect(dynamicDemo.locator('p')).toContainText('当前进度：50%')
    await expect(dynamicDemo.locator('.weui-progress__inner-bar')).toHaveCSS('width', '50%')
    // 再点击两次直到 90%
    await dynamicDemo.locator('.weui-btn_primary').click()
    await dynamicDemo.locator('.weui-btn_primary').click()
  ...
  test('动态进度：上限为 100%', async ({ page, gotoDocsPage }) => {
    ...
    await expect(dynamicDemo.locator('.weui-progress__inner-bar')).toHaveCSS('width', '100%')
  })
  ...
  test('组合使用：同时设置多个属性', async ({ page, gotoDocsPage }) => {
    ...
    await expect(innerBar).toHaveCSS('width', '75%')
  })
})
